...
const ZWETH: React.FC = () => {
  ...
  const [tokenDecimals] = useState<number>(18); // ETH always uses 18 decimals
  const [ethBalance, setEthBalance] = useState<string>('0');
  const [zwethBalance, setZwethBalance] = useState<string>('0');
  ...
  const refreshBalances = React.useCallback(async () => {
    ...
    try {
      ...
      // Query ZWETH balance
      const zwethContract = new ethers.Contract(
        CONTRACT_ADDRESSES.ZWETH,
        CONTRACT_ABIS.ZWETH,
        provider,
      );
      const zwethBal = await zwethContract.balanceOf(account);
      setZwethBalance(ethers.formatEther(zwethBal));
    } catch (error) {
  ...
  const handleGenerateBySeed = async (targetMode?: 'deposit' | 'transfer' | 'advancedDeposit') => {
    ...
    try {
      ...
      const signMessage = `ZWETH: ${CONTRACT_ADDRESSES.ZWETH}, chainId: ${network.chainId}`;
  ...
  // ETH doesn't need approval, so these are always false
  const simpleNeedsApproval = false;
  const advancedNeedsApproval = false;
  ...
  const handleSimpleDeposit = async (values: { amount: number; targetAddress: string }) => {
    ...
    try {
      ...
      const depositAmountBigInt = ethers.parseEther(values.amount.toString());
      console.log(
        `[Simple] Deposit amount: ${
          values.amount
        } ETH = ${depositAmountBigInt.toString()} wei`,
      );

      // Execute deposit with targetAddress (Burn) - include ETH value
      const zwethContract = new ethers.Contract(
        CONTRACT_ADDRESSES.ZWETH,
        CONTRACT_ABIS.ZWETH,
        signer,
      ...
      const tx = await zwethContract.deposit(values.targetAddress, 0, depositAmountBigInt, '0x', {
  ...
  const handleAdvancedDeposit = async (values: { amount: number; targetAddress?: string }) => {
    ...
    try {
      ...
      console.log('[Advanced] Proceeding to wrap...');
      ...
      const tx = await zwethContract.deposit(toAddress, 0, depositAmountBigInt, '0x', {
        value: depositAmountBigInt,
      });
  ...
  const handleWithdraw = async (values: { amount: number }) => {
    ...
    try {
      ...
      // Use ETH decimals (18)
      const withdrawAmount = ethers.parseEther(values.amount.toString());
      console.log(
        `Withdraw amount: ${
          values.amount
        } ETH = ${withdrawAmount.toString()} wei`,
      );
  ...
  const handleTransfer = async (values: { targetAddress: string; amount: number }) => {
    ...
    try {
      ...
      console.log(
        `Transfer amount: ${
          values.amount
        } ETH = ${transferAmount.toString()} wei`,
      );
  ...
  const handleRemint = async (values: any) => {
    ...
    try {
      ...
      const remintAmount = ethers.parseEther(values.remintAmount.toString());
      console.log(
        `Remint amount: ${
          values.remintAmount
        } ETH = ${remintAmount.toString()} wei`,
      );
          ...
          >
            ...
            <div style={{ flex: '1 1 200px', minWidth: '200px' }}>
              ...
              >
                {account ? (
                  <>
                    {parseFloat(zwethBalance).toFixed(6)}{' '}
                    <a
                      href={`https://sepolia.etherscan.io/address/${CONTRACT_ADDRESSES.ZWETH}`}
                      target="_blank"
                      rel="noopener noreferrer"
                      style={{
                        color: '#fff',
                  ...
                  >
                    ...
                    {selectedRemintMaxAmount && parseFloat(selectedRemintMaxAmount) > 0 && (
                      ...
                      >
                        ...
                        {parseFloat(selectedRemintMaxAmount).toFixed(6)} ETH
                      </div>
                    )}
                ...
                <div style={{ maxWidth: 600, margin: '0 auto', padding: '24px 0' }}>
                  <Form form={remintForm} layout="vertical" onFinish={handleRemint}>
                    ...
                    {selectedRemintMaxAmount && parseFloat(selectedRemintMaxAmount) > 0 && (
                      ...
                      >
                        💡 {intl.formatMessage({ id: 'pages.zwtoken.remint.maxAmountTip' })}:{' '}
                        {parseFloat(selectedRemintMaxAmount).toFixed(6)} ZWETH
                      </div>
                    )}
                ...
                },
                {
                  ...
                  render: (amount: string, record) => {
                    ...
                    if (record.isClaimed) {
                      return (
                        <span style={{ color: '#999' }}>
                          0 ETH ({intl.formatMessage({ id: 'pages.zwtoken.table.claimed' })})
                        </span>
                      );
                    }
                    return <span style={{ color: '#52c41a' }}>0 ETH</span>;
                  },
                },
                {
                ...
                },
                {
                  ...
                  render: (amount: string, record) => {
                    ...
                    if (amountNum > 0) {
                      return (
                        <span style={{ color: '#faad14', fontWeight: 'bold' }}>
                          {parseFloat(amount).toFixed(6)} ZWETH
                        </span>
                      );
                    }
                    // Show different message based on isClaimed status
                    if (record.isClaimed) {
                      return (
                        <span style={{ color: '#999' }}>
                          0 ZWETH ({intl.formatMessage({ id: 'pages.zwtoken.table.claimed' })})
                        </span>
                      );
                    }
                    return <span style={{ color: '#52c41a' }}>0 ZWETH</span>;
                  },
                },
                {
                  title: intl.formatMessage({ id: 'pages.zwtoken.table.isClaimed' }),
                ...
                },
                {
                  ...
                  render: (amount: string, record) => {
                    ...
                    if (record.isClaimed) {
                      return (
                        <span style={{ color: '#999' }}>
                          0 ETH ({intl.formatMessage({ id: 'pages.zwtoken.table.reminted' })})
                        </span>
                      );
                    }
                    return <span style={{ color: '#52c41a' }}>0 ETH</span>;
                ...
                },
                {
                  ...
                  render: (amount: string, record) => {
                    ...
                    if (record.isClaimed) {
                      return (
                        <span style={{ color: '#999' }}>
                          0 ZWETH ({intl.formatMessage({ id: 'pages.zwtoken.table.reminted' })})
                        </span>
                      );
                    }
                    return <span style={{ color: '#52c41a' }}>0 ZWETH</span>;
